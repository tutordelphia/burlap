...
def get_owner(path, use_sudo=False):
    """
...
def get_group(path, use_sudo=False):
    """
...
def get_mode(path, use_sudo=False):
    """
    ...
    def require(self, path=None, contents=None, source=None, url=None, md5=None,
        ...
        *group* and *mode* parameters can be used to override these properties.
        ...
        if use_sudo and owner is None:
            owner = 'root'
        if (owner and get_owner(path, use_sudo) != owner) or \
           (group and get_group(path, use_sudo) != group):
            func('chown %(owner)s:%(group)s "%(path)s"' % locals())
    
        # Ensure correct mode
        if use_sudo and mode is None:
            mode = oct(0666 & ~int(umask(use_sudo=True), base=8))
        if mode and get_mode(path, use_sudo) != mode:
