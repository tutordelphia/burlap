...
def debian_family():
    with patch('burlap.apache.distrib_family') as mock:
...
def debian(debian_family):
    with patch('burlap.apache.distrib_id') as mock:
        mock.return_value = 'Debian'
        yield
...
def debian_8_0(debian):
    with patch('burlap.apache.distrib_release') as mock:
